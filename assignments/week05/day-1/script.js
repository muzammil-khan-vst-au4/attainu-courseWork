$(document).ready(function() {
	...
	$.get('currency.json', {}, function(data) {
		for(var currencyCode in data.rates) {
			$('<option/>').val(currencyCode).text(currencyCode).appendTo('.currency-one');
			$('<option/>').val(currencyCode).text(currencyCode).appendTo('.currency-two');
		}
	});
